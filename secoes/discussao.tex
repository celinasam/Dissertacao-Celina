...
No esquema da codificação, para o mesmo objetivo, para as mesmas probabilidades $e$ e $q$, obtemos um valor de 3 blocos de paridade para uma codificação $(2k-1, k)$.
...
No HDFS, a codificação RS implementada é (16, 10), 10 blocos iniciais e 6 blocos de paridade. Logo, tolera até $l=6$ falhas por \emph{stripe} de 10 blocos ($k=10$) com sobrecarga de $B=1.6$, como a replicação de $n=6$ réplicas, que tem sobrecarga de $B=6$. A codificação RAID implementada é RAID-5, portanto, é (12, 10), 10 blocos iniciais e 2 blocos de paridade. Logo, tolera $l=2$ falhas por \emph{stripe} de 10 blocos ($k=10$) com sobrecarga de $B=1.2$ como a replicação de $n=2$ réplicas, que tem sobrecarga de $B=2$.
