...
No HDFS, a codificação RS \emph{default} é (5, 3).  Logo, tolera até $l=3$ falhas por \emph{stripe} de 5 blocos ($k=5$) com sobrecarga de $B=1.6667$, como a replicação de $n=3$ réplicas, que tem sobrecarga de $B=3$. 

No HDFS, a codificação RAID implementada é RAID-5 com tamanho \emph{default} da \emph{stripe} de 5 blocos, logo, tolera $l=2$ falhas por \emph{stripe} de 5 blocos ($k=5$) com sobrecarga de $B=2.5$ como a replicação de $n=2$ réplicas, que tem sobrecarga de $B=2$.

Nas máquinas do Facebook, a codificação RS tem 10 blocos iniciais e 6 blocos de paridade ($B=1.6$) e a codificação RAID é (12, 10), 10 blocos iniciais e 2 blocos de paridade ($B=1.2$).
