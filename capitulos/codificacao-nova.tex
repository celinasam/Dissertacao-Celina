...
A idéia básica da código ótimo é que o objeto original possa ser reconstruído a partir de quaisquer $m$ únicos fragmentos que são aproximadamente do mesmo tamanho do objeto original \cite{Weatherspoon:2002:01}.
...
Códigos de blocos são identificados pela notação ($n$, $k$), de acordo com o número de \emph{bits} de saída $n$ e o número de \emph{bits} $k$ de cada um dos  blocos de entrada.
...
$\frac{(n - k)}{n}$. A alta redundância reduz a possibilidade de
...
\begin{description}
   ...
   \item [distância mínima] $(d_{min})$ é a menor distância de Hamming \footnote{A distância de Hamming (dH) entre duas palavras código $v_i$ e $v_j$  é o número de \emph{bits} que são diferentes nessas duas palavras.} entre duas quaisquer palavras do código; ela depende do número de \emph{bits} redundantes $q = n - k$, tal que  $(d_{min} \leq q + 1)$
...
Códigos Reed-Solomon são códigos de blocos, lineares e cíclicos.
