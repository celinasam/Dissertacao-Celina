...
\begin{definition}
Seja $u=u_0u_1 \ldots u_k \in {\mathbb{Z}_2}^k$. O tamanho de $u$ é dado por 
\begin{align*}
...
\begin{proposition} Suponha que $u, v \in {\mathbb{Z}_2}^k$. Então $\delta(u,v)=\omega(u+v)$.
\end{proposition}

\begin{proof} Seja $P(n)$ a proposição que $\delta(u,v)=\omega(u+v)$ para $\forall u,v \in {\mathbb{Z}_2}^n$.

Base: $P(1)$ é verdadeira, pois $\delta(0,0)=\omega(0+0)=0$, $\delta(0,1)=\omega(0+1)=1$, $\delta(1,0)=\omega(1+0)=1$ e $\delta(1,1)=\omega(1+1)=0$.
...
\end{align*}
Sabemos calcular $\delta(u_{k+1},v_{k+1})$, pois $P(1)$ é verdadeira. Pela hipótese de indução, $\delta(u,v)=\omega(u+v)$. Então, $\delta(u^{'},v^{'})=\omega(u,v)+\omega(u_{k+1},v_{k+1})=\omega(u^{'},v^{'})$$\square$
\end{proof}

\begin{proposition} \label{somauns} Suponha que $u, v \in {\mathbb{Z}_2}^k$. Então $\omega(u+v) \leq \omega(u) + \omega(v)$
\end{proposition}
...
$$
\begin{array}{l}
u,v\ \ \ \ u+v\ \ \ \ \omega (u+v)\ \leq \omega(u) + \omega(v)\\
00,00\ \ \ 00\ \ \ \ \ \ 0\ \ \ \ \ \ \ \ \ \ \ \ \  0\\
00,01\ \ \ 01\ \ \ \ \ \ 1\ \ \ \ \ \ \ \ \ \ \ \ \ 0+1=1\\
00,10\ \ \ 10\ \ \ \ \ \ 1\ \ \ \ \ \ \ \ \ \ \ \ \ 0+1=1\\
00,11\ \ \ 11\ \ \ \ \ \ 0\ \ \ \ \ \ \ \ \ \ \ \ \ 0\\
...
$$
Passo de indução: Suponhamos que P(k) é verdadeira $\forall (u,v) \in {\mathbb{Z}_2}^k$ e para $\forall k < n$.
Queremos provar, para $k+1$, que P(k+1) é verdadeira. Seja $u=u_0u_1 \ldots u_k \in {\mathbb{Z}_2}^k$, $v=v_0v_1 \ldots v_k \in {\mathbb{Z}_2}^k$ e $z=u+v=z_0z_1 \ldots z_k \in {\mathbb{Z}_2}^k$ e $u^{'}=u_0u_1 \ldots u_ku_{k+1} \in {\mathbb{Z}_2}^k$, $v^{'}=v_0v_1 \ldots v_kv_{k+1} \in {\mathbb{Z}_2}^k$ e $z^{'}=u+v=z_0z_1 \ldots z_kz_{k+1} \in {\mathbb{Z}_2}^{k+1}$. Podemos escrever $\omega (u^{'}+v^{'})=\omega (z^{'})$ como
\begin{align*}
\omega (z^{'})=\omega(z)+\omega(u_{k+1}+v_{k+1})=\omega(u+v)+\omega(u_{k+1}+v_{k+1})
\end{align*}
Sabemos calcular $\omega(u_{k+1}+v_{k+1})$, pois $P(2)$ é verdadeira. Então $\omega(u_{k+1}+v_{k+1}) \leq \omega(u_{k+1})+\omega(v_{k+1})$. Pela hipótese de indução, $\omega(u+v) \leq \omega(u) + \omega(v)$. Então
\begin{align*}
\omega (z^{'}) =\omega(u+v)+\omega(u_{k+1}+v_{k+1}) \leq \omega(u) + \omega(v) + \omega(u_{k+1})+ \omega(v_{k+1})
\end{align*}
$\square$
\end{proof}
...
\section{Exemplos de Códigos Corretores de Erros}

\begin{definition} Seja uma sequência de bits $w=w_0w_1 \ldots w_{n-1} \in \{0, 1\}^n$. Podemos definir $w$ como sendo um elemento $w=(w_0w_2 \ldots w_{n-1})$ do produto cartesiano:
\begin{align*}
