...
\begin{definition} {\bf Congruência} \index{Congruência} Seja $q \in \mathbb{N}$. Dois inteiros $a$
 e $b$ dizem-se $congruentes\ modulo\ q$ se tiverem o mesmo resto na divisão por
 $q$. A notação é $a \equiv b(mod\ q)$. Daí temos que $a = qk + b$, para um $k \in \mathbb{N}$.
\end{definition}
...
\begin{definition} {\bf Classe Residual} \index{Classe Residual} Seja $q \in \mathbb{N}$ e $q > 1$. A classe
...
\begin{theorem} {\bf Divisão Euclidiana} \label{DivEucli} \index{Divisão Euclidiana} Dados $a \in \mathbb{Z}$, $b \in \mathbb{Z}$ e $b \neq 0$, $\exists c,\ \exists r$ inteiros únicos $\in\ \mathbb{Z}$ tais que $a\ =\ bc\ +\ r$ e $0 \leq r < |b|$.
\end{theorem}

$c$ e $r$ são chamados, respectivamente, o quociente e resto da divisão de a por b.
...
\begin{proposition}
Todo o corpo finito tem $p^i$ elementos para algum primo $p$ e algum natural $i$.
\end{proposition}
...
\begin{proposition}
Para cada primo $p$ e cada natural $i$, existe um corpo com $p^i$ elementos.
\end{proposition}
...
\begin{proposition}
Qualquer corpo com $p^i$ elementos é isomorfo à extensão de decomposição de $x^q - x, q = p^i$ sobre $GF(q)$.
\end{proposition}
...
\begin{example} O $GF(8)$ ilustrado nas tabelas~\ref{tab5:comp} e~\ref{tab6:comp}.
...
\end{proof}
		...
		$GF [q](x)$ é um corpo de Galois que tem ordem $q$, sobre o qual são aplicados polinômios com coeficientes entre $0$ e $q - 1$. Um exemplo de um polinômio $p(x)$ pode ser apresentado na equação: $p(x) = a_7.x^7 + a_5.x^5 + a_2.x^2 + ax + 1$. Os coeficientes de um polinômio $a_i$ pertencem ao $GF(p)$.
